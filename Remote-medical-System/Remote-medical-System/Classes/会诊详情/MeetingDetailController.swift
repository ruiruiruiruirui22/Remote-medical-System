    ...
    private func initUI(){
        ...
        case 0:
            ...
            if viewModel.applyDoctor.id == User.shareInstance.userid {
                ...
                if cancelMeetingBtn != nil {
                    cancelMeetingBtn.backgroundColor = UIColor(displayP3Red: 255/255, green: 172/255, blue: 75/255, alpha: 1)
                    cancelMeetingBtn.setTitle("拒绝申请", for:.normal)
                    cancelMeetingBtn.addTarget(self, action: #selector(rejectApply), for: .touchUpInside)
                }
                
                if enterVideoBtn != nil {
                    enterVideoBtn.setTitle("接受申请", for:.normal)
                    enterVideoBtn.addTarget(self, action: #selector(receiveApply),     for: .touchUpInside)
                }
            }
        case 1:
            state = "当前状态：专家已确认"
            ...
            if enterVideoBtn != nil {
                enterVideoBtn.setTitle("进入视频", for:.normal)
                enterVideoBtn.addTarget(self, action: #selector(enterVideo), for: .touchUpInside)
            }
        case 2:
            state = "当前状态：专家已拒绝"
            if cancelMeetingBtn != nil {
        ...
        case 3:
            ...
            if enterVideoBtn != nil {
                enterVideoBtn.setTitle("讨论区", for:.normal)
                enterVideoBtn.addTarget(self, action: #selector(haveatalk), for: .touchUpInside)
            }
        default:
            state = "当前状态：会议已取消"
        }
...
extension MeetingDetailController{
    ...
    @objc private func haveatalk(){
        let sb = UIStoryboard(name: "meetingDetail", bundle: nil)
        let vc = sb.instantiateViewController(withIdentifier: "talkView") as! TalkViewController
        vc.chatRoomId = "\(viewModel?.meeting.id ?? 0)"
        vc.userName = User.shareInstance.username
        self.present(vc, animated: true, completion: nil)
    }
    
    @objc private func rejectApply(){
        let sb = UIStoryboard(name: "reject", bundle: nil)
        let vc = sb.instantiateViewController(withIdentifier: "rejectView") as! RejectViewController
        vc.meeting_id = viewModel?.meeting.id
        vc.doctor_id = User.shareInstance.userid
        self.present(vc, animated: true, completion: nil)
    }
    ...
    @objc private func enterVideo(){
        let sb = UIStoryboard(name: "video", bundle: nil)
        let vc = sb.instantiateViewController(withIdentifier: "videoView") as! VideoViewController
        self.present(vc, animated: true, completion: nil)
    }

...
class MyMeetingController: BaseViewController,UIPickerViewDelegate, UIPickerViewDataSource,UITableViewDelegate,UITableViewDataSource,UISearchBarDelegate{
    ...
    //MARK: -ui控件
    @IBOutlet var applyView: UIImageView!
    @IBOutlet var applyedView: UIImageView!
    ...
    @IBOutlet var applySelect: UIImageView!
    @IBOutlet var applyedSelect: UIImageView!
    ...
    private func initUI(){
        ...
        applyView.isUserInteractionEnabled = true
        let tapGesture1 = UITapGestureRecognizer(target: self, action: #selector(applyViewClick))
        tapGesture1.numberOfTapsRequired = 1
        applyView.addGestureRecognizer(tapGesture1)
        
        applyedView.isUserInteractionEnabled = true
        let tapGesture2 = UITapGestureRecognizer(target: self, action: #selector(applyedViewClick))
        tapGesture2.numberOfTapsRequired = 1
        applyedView.addGestureRecognizer(tapGesture2)
...
extension MyMeetingController{
    @objc private func applyViewClick(){
         flag = !flag
       applySelect.isHidden = false
       applyedSelect.isHidden = true
        loadMeeting()
    }
    
    @objc private func applyedViewClick(){
         flag = !flag
        applySelect.isHidden = true
        applyedSelect.isHidden = false
          loadMeeting()
    }
